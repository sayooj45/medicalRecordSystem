...
const PatientProfileData = ({profileData}) => {
  ...
  const testReportSubmit = async (e) => {
  // e.preventDefault();
  try {
    ...
    const res = await fetch("/api/user/profile/report", {
      method: "POST",
      credentials: "include",
      body: formData, 
    });
...
const prescriptionSubmit = async (e) => {
  ...
  try {
    ...
    const res = await fetch("/api/user/prescription", {
      method: "POST",
      credentials: "include",
      body: formData, 
    });
          ...
          <div className="flex justify-around">
            ...
            >
              ...
              <h1 className="font-bold text-[24px]">
                {profileData.heartRate}<span className="text-[16px]">bps</span>
              </h1>
            </div>
            <div
              className="bg-white w-[300px] h-[200px] flex flex-col justify-center items-center rounded-2xl relative"
            >
              ...
              <h1 className="font-bold text-[24px]">{profileData.Temperature}°c</h1>
            </div>
            <div
              className="bg-white w-[300px] h-[200px] flex flex-col justify-center items-center rounded-2xl relative"
            >
              ...
              <h1 className="font-bold text-[24px]">{profileData.glucose}mg/dl</h1>
            </div>
          </div>
...
<div className="bg-white p-5 rounded-2xl mt-[50px] mx-[60px] w-[1100px] ">
  ...
  <div className="relative w-full flex justify-center items-center overflow-hidden">
    ...
    <div className="overflow-hidden w-[90%]">
      ...
      >
        {profileData?.testReport?.map((data, index) => (
          <div
            key={index}
    ...
    <button
      onClick={() =>
        setCurrentIndex((prev) =>
          prev + 1 < (profileData?.testReport?.length || 0) - 2
            ? prev + 1
            : prev
        )
      }
      disabled={currentIndex + 3 >= (profileData?.testReport?.length || 0)}
  ...
  <div className="fixed inset-0 z-50 flex justify-center items-center backdrop-blur-2xl bg-black/30">
    <div className="p-10 bg-white rounded-lg shadow-lg w-[600px] max-w-[90%]">
      ...
      <form onSubmit={(e) => {  testReportSubmit(); }}>
    ...
    <div className="p-10 bg-white rounded-lg shadow-lg w-[600px] max-w-[90%]">
      ...
      <form onSubmit={ prescriptionSubmit}>
        ...
        <div className="flex justify-center mt-8">
          ...
          <div className="bg-white p-5 rounded-2xl mt-[50px] mx-[60px]">
            <h1 className="text-[24px] font-bold mb-5">Prescriptions</h1>
            {
              loginDetails?(
                loginDetails.role === 'patient'?
              <button
              className="text-[#66D0C6] border border-[#66D0C6] w-full h-[75px] rounded-lg font-bold text-[24px]"
            ...
            <div className="h-[200px] overflow-auto">
              ...
              >
                ...
                {
                    profileData.prescription?.map((data,index)=>(
                      <tbody>
                    <tr>
                  <td>
